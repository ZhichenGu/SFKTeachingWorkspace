...
window.SFKBatch=(()=>{
  ...
  view.innerHTML='<div class="batch-bar"><div><h2>批量新建签单</h2><p class="batch-status">一行一个学生；顶部是本批次默认值，行内留空即继承默认。</p></div><div style="display:flex;gap:10px;flex-wrap:wrap"><button type="button" id="batch-back">← 返回管理</button></div></div>'+
   ...
   '<div class="batch-list"><div class="batch-pager" id="batch-pager"><button type="button" id="pg-prev">上一份</button><span id="pg-count" class="batch-status">第 1 / 1 份</span><button type="button" id="pg-next">下一份</button><button type="button" id="pg-add">＋ 新增</button></div><div class="batch-head"><span>学生姓名</span><span>课程</span><span>日期</span><span>时间（开始/结束）</span><span>上课内容</span><span>课后作业</span><span>操作</span></div><div id="batch-rows"></div></div>'+
  ...
  function renderRow(row,index){
    ...
    const tl1=document.createElement('span');tl1.textContent='开始';tl1.style.cssText='font-size:11px;color:#69748a;display:block;margin-bottom:2px';
    const tl2=document.createElement('span');tl2.textContent='结束';tl2.style.cssText='font-size:11px;color:#69748a;display:block;margin-bottom:2px';
    const tinWrap=el('div');tinWrap.append(tl1,tin);const toutWrap=el('div');toutWrap.append(tl2,tout);
    const tcell=el('div','cell-time');tcell.append(tinWrap,toutWrap);
    ...
    const numCell=el('div','cell','第几次课');numCell.append(num);
    const prevCell=el('div','cell','作业完成');prevCell.append(prev);
    sub.append(numCell,prevCell);
    const cells=[[name,'学生'],[course,'课程'],[date,'日期'],[tcell,'时间'],[content,'上课内容'],[homework,'课后作业'],[ops,'操作']];
